...
import { tractorTrukModel } from "../../../../DB/model/TractorTractor.js";
...
import cloudinary from "../../../services/cloudinary.js";
 ...
 export const addModel=async(req,res)=>{
    try{
const {name}=req.body
const find =await tractorTrukModel.findOne({name:name});
if(find){
    res.status(400).json({message:"النموذج موجود مسبقا"})
}else{
    const add= new tractorTrukModel({name:name});
...
export const addQuestionToModel = async (req, res) => {
    try {
        ...
        const newQuestion = await tractorModel.create({
            question,
            options: newOptions,
        ...
        const updatedModel = await tractorTrukModel.findOneAndUpdate(
            { _id: modelId },
 ...
 export const deletModel=async(req,res)=>{
    const {id}=req.params;
    const findModel=await tractorTrukModel.findByIdAndDelete(id);
    if(!findModel){
...
export const deletQuestionFromModel=async(req,res)=>{
    try{
        ...
        const deletQuestion=await tractorModel.findById(questionId);
    
        if(!deletQuestion){
            res.status(400).json({message:"السؤال مش موجود"})
        }else{
            const delet=await tractorTrukModel.findByIdAndUpdate(modelId,{$pull:{questions:deletQuestion}},{new:true})
 ...
 export const showQuestioninModel=async(req,res)=>{
   const {id }=req.params;
   const findModel=await tractorTrukModel.findById(id);
   if(!findModel){
...
export const showModel=async(req,res)=>{
    try{
        const findModel=await tractorTrukModel.find({});
        if(!findModel){
