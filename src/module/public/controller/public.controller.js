...
export const createPulic = async (req, res) => {
    try {
        const { question, image, options, correctAnswer } = req.body;

        const findQuestion = await publicModel.findOne({ question: question })
        ...
        const questions = await publicModel.create(req.body)
...
export const deletPublic = async (req, res) => {
    try {
        const { id } = req.params
        const findQuestion = await publicModel.findByIdAndDelete(id);
        if (!findQuestion) {
...
export const getAll=async(req,res)=>{
    const getAllPrivate=await publicModel.find({})
    if(!getAllPrivate){
...
export const update = async (req, res) => {
    try {
      ...
      const findMicro = await publicModel.findById(id);
      ...
      const update = await publicModel.findByIdAndUpdate(id, updatedData, { new: true });
