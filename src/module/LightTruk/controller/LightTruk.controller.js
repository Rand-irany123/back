...
import { lightModel } from "../../../../DB/model/light.model.js";
import { lightTrukModel } from "../../../../DB/model/lightTruck.js";
...
import cloudinary from "../../../services/cloudinary.js";
 ...
 export const addModel=async(req,res)=>{
    try{
const {name}=req.body
const find =await lightTrukModel.findOne({name:name});
if(find){
    res.status(400).json({message:"النموذج موجود مسبقا"})
}else{
    const add= new lightTrukModel({name:name});
        ...
        try {
            ...
            const newOptions = await Promise.all(options.map(async (option, index) => {
                ...
                return {
                    text: text,
                    image: optionImage,
                    code: String.fromCharCode(97 + index) // تحويل الفهرس إلى حرف (a, b, c, d)
                };
            }));
            ...
            if (correctAnswer.text || req.files && req.files['correctAnswer[image]']) {
                correctAnswerData = {
                    ...
                    image: null,
                    code: correctAnswer.code // رمز الإجابة الصحيحة
                };
            ...
            const newQuestion = await lightModel.create({
                question,
                options: newOptions,
            ...
            const updatedModel = await lightTrukModel.findOneAndUpdate(
                { _id: modelId },
 ...
 export const deletModel=async(req,res)=>{
    const {id}=req.params;
    const findModel=await lightTrukModel.findByIdAndDelete(id);
    if(!findModel){
...
export const deletQuestionFromModel=async(req,res)=>{
    try{
        ...
        const deletQuestion=await lightModel.findById(questionId);
    
        if(!deletQuestion){
            res.status(400).json({message:"السؤال مش موجود"})
        }else{
            const delet=await lightTrukModel.findByIdAndUpdate(modelId,{$pull:{questions:deletQuestion}},{new:true})
 ...
 export const showQuestioninModel=async(req,res)=>{
   const {id }=req.params;
   const findModel=await lightTrukModel.findById(id);
   if(!findModel){
...
export const showModel=async(req,res)=>{
   const findModel=await lightTrukModel.find({});
   if(!findModel){
