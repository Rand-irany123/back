...
export const createMicro = async (req, res) => {
    try {
       
        const { question, image, options, correctAnswer } = req.body;

        const findQuestion = await microModel.findOne({ question: question })
        ...
        const questions = await microModel.create(req.body)
...
export const deletmicro = async (req, res) => {
    try {
        const { id } = req.params
        const findQuestion = await microModel.findByIdAndDelete(id);
        if (!findQuestion) {
...
export const getAll=async(req,res)=>{
    const getAllPrivate=await microModel.find({})
    if(!getAllPrivate){
...
export const update = async (req, res) => {
    try {
      ...
      const findMicro = await microModel.findById(id);
  
      if (!findMicro) {
        return res.status(400).json({ message: "المايكرو غير موجود." });
      }
      ...
      const update = await microModel.findByIdAndUpdate(id, updatedData, { new: true });
