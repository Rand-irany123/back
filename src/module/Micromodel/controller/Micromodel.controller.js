...
import { microModel } from "../../../../DB/model/micro.model.js";
import { MicrotrukModel } from "../../../../DB/model/microModel.js";
...
import cloudinary from "../../../services/cloudinary.js";
 ...
 export const addModel=async(req,res)=>{
    try{
const {name}=req.body
const find =await MicrotrukModel.findOne({name:name});
if(find){
    res.status(400).json({message:"النموذج موجود مسبقا"})
}else{
    const add= new MicrotrukModel({name:name});
...
export const addQuestionToModel = async (req, res) => {
    try {
        ...
        const newQuestion = await microModel.create({
            question,
            options: newOptions,
        ...
        const updatedModel = await MicrotrukModel.findOneAndUpdate(
            { _id: modelId },
 ...
 export const deletModel=async(req,res)=>{
    const {id}=req.params;
    const findModel=await MicrotrukModel.findByIdAndDelete(id);
    if(!findModel){
...
export const deletQuestionFromModel=async(req,res)=>{
    try{
        ...
        const deletQuestion=await microModel.findById(questionId);
    
        if(!deletQuestion){
            res.status(400).json({message:"السؤال مش موجود"})
        }else{
            const delet=await MicrotrukModel.findByIdAndUpdate(modelId,{$pull:{questions:deletQuestion}},{new:true})
 ...
 export const showQuestioninModel=async(req,res)=>{
   const {id }=req.params;
   const findModel=await MicrotrukModel.findById(id);
   if(!findModel){
...
export const showModel=async(req,res)=>{
   const findModel=await MicrotrukModel.find({});
   if(!findModel){
