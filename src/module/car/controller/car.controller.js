...
import cloudinary from "../../../services/cloudinary.js";
 ...
 export const addModel=async(req,res)=>{
    try{
const {name}=req.body
const find =await carModel.findOne({name:name});
if(find){
    res.status(400).json({message:"النموذج موجود مسبقا"})
}else{
    const add= new carModel({name:name});
...
export const addQuestionToModel = async (req, res) => {
    try {
        ...
        const updatedModel = await carModel.findOneAndUpdate(
            { _id: modelId },
 ...
 export const deletModel=async(req,res)=>{
    const {id}=req.params;
    const findModel=await carModel.findByIdAndDelete(id);
    if(!findModel){
...
export const deletQuestionFromModel=async(req,res)=>{
    try{
        ...
        const deletQuestion=await QuestionModel.findById(questionId);
    
        if(!deletQuestion){
            res.status(400).json({message:"السؤال مش موجود"})
        }else{
            const delet=await carModel.findByIdAndUpdate(modelId,{$pull:{questions:deletQuestion}},{new:true})
 ...
 export const showQuestioninModel=async(req,res)=>{
   const {id }=req.params;
   const findModel=await carModel.findById(id);
   if(!findModel){
...
export const showModel=async(req,res)=>{
   const findModel=await carModel.find({});
   if(!findModel){
