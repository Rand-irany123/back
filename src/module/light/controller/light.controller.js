...
export const createlight = async (req, res) => {
    try {
       
        const { question, image, options, correctAnswer } = req.body;

        const findQuestion = await lightModel.findOne({ question: question })
        ...
        const questions = await lightModel.create(req.body)
...
export const deletlight = async (req, res) => {
    try {
        const { id } = req.params
        const findQuestion = await lightModel.findByIdAndDelete(id);
        if (!findQuestion) {
...
export const getAll=async(req,res)=>{
    const getAllPrivate=await lightModel.find({})
    if(!getAllPrivate){
...
export const update = async (req, res) => {
    try {
      ...
      const findMicro = await lightModel.findById(id);
  
      if (!findMicro) {
        return res.status(400).json({ message: "ا غير موجود." });
      }
      ...
      const update = await lightModel.findByIdAndUpdate(id, updatedData, { new: true });
